...
layui.define(['laytpl', 'layer'], function (exports) {
  ...
  Class.prototype.render = function (views, params) {
    var that = this,
      router = layui.hash();
    views =
      (setter.paths && setter.paths.views ? setter.paths.views : setter.views) +
      views +
      setter.engine;
  ...
  Class.prototype.parse = function (html, refresh, callback) {
    ...
    for (var i = elemTemp.length; i > 0; i--) {
      (function () {
        ...
        var layUrl = laytpl(dataElem.attr('lay-url') || '').render(hash); // 接口 url
        var data = laytpl(dataElem.attr('lay-data') || '').render(hash); // 接口参数
        var headers = laytpl(dataElem.attr('lay-headers') || '').render(hash); // 接口请求的头信息
  ...
  Class.prototype.refresh = function (callback) {
    ...
    that.parse(that.container, 'refresh', function () {
      that.container
        .siblings('[lay-templateid="' + that.id + '"]:last')
        .remove();
      typeof callback === 'function' && callback();
    });
