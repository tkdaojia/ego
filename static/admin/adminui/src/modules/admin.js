...
layui.define('view', function (exports) {
  ...
  var admin = {
        ...
        list.find('li,dd').each(function (index, item) {
          ...
          var matched =
            ...
            (function () {
              ...
              return hrefPath !== '/' && url.href.indexOf(hrefPath) !== -1;
            })();
    ...
    theme: function (options) {
      ...
      ).render((options = $.extend({}, local.theme, options)));
    ...
    serach: function (othis) {
      othis.off('keypress').on('keypress', function (e) {
        ...
        if (e.keyCode === 13) {
          ...
          var title =
            othis.attr('lay-title') || othis.attr('lay-text') || '搜索';
  ...
  admin.on('tabsPage(setMenustatus)', function (router) {
    var url = layui.hash('#' + admin.correctRouter(router.url));
    admin.setSideMenuStatus(url);
  });

  // 事件-侧边导航点击事件
  element.on('nav(layadmin-system-side-menu)', function (elem) {
    if (
      elem.siblings('.layui-nav-child')[0] &&
  ...
  $body.on('click', '*[lay-href]', function () {
    ...
    var title =
      othis.attr('lay-title') || othis.attr('lay-text') || othis.text();
    ...
    if (setter.pageTabs && setter.refreshCurrPage) {
      if (
        href === admin.tabsBody(admin.tabsPage.index).find('iframe').attr('src')
      ) {
  ...
  $body
    .on('mouseenter', '*[lay-tips]', function () {
      ...
      if (
        othis.parent().hasClass('layui-nav-item') &&
        !container.hasClass(SIDE_SHRINK)
      )
        return;
